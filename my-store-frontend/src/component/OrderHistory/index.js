...
const { SlArrowDown, SlArrowUp } = icons;
  ...
  return (
    <div className="min-h-screen w-full gap-4 flex flex-col items-center my-20">
      ...
      {ordersList?.map((order, index) => (
        <div
          className="w-full 768:w-3/5 border-collapse border rounded-md duration-500 shadow-md flex flex-col gap-3 p-4 cursor-pointer"
          key={index}
          onClick={() => handleToggleDetailCart(order[0].orderId)}
        >
          <div className="flex justify-between items-center ">
            ...
            <span className="text-gray-400 text-sm">
              {moment(order[0]).format("DD-MM-YYYY")}
            </span>
            <span>
          ...
          {selectedOrderIds.includes(order[0].orderId) &&
            order.map((product) => (
              <div
                key={product.id}
                className="px-4 flex gap-4 duration-1000 animate-slide-left2"
              >
                <img
                  src={product && JSON.parse(product?.image)[0]?.url}
                  alt="thumbnail"
          ...
          <span className="text-lg font-medium">
            Tổng tiền:{" "}
            {handleSumTotalPrice(order).toLocaleString("en-US", {
              style: "currency",
              currency: "VND",
            })}
          </span>
      ...
      {isFetching &&
        ReactDOM.createPortal(<Loading />, document.getElementById("loading"))}
    </div>
  );
};
